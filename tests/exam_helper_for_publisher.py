...
class BasePublisherTestCase(WithTestUserTestCase):
    _wallet_base_uri = env("E2E_PUBLISHER_WALLET_BASE_URI", required=False, default="http://localhost:8081")
    _wallet_admin_client_id = env(
        "E2E_PUBLISHER_WALLET_CLIENT_ID", required=False, default="data-lake-frontend-e2e-test"
    )
    _wallet_admin_client_secret = env(
        "E2E_PUBLISHER_WALLET_CLIENT_SECRET", required=False, default="dev-secret-never-use-in-prod"
    )
    ...
    collection_service_base_url = env(
        "E2E_COLLECTION_SERVICE_BASE_URL", required=False, default="http://localhost:8093"
    )

    publisher_wallet_admin_group = env(
        "E2E_PUBLISHER_WALLET_ADMIN_GROUP", required=False, default="data-lake-active-admins"
    )
    publisher_wallet_test_users_group = env(
        "E2E_PUBLISHER_WALLET_TEST_USERS_GROUP", required=False, default="data-lake-test-users"
    )
    ...
    def do_on_setup_class_before_auth(cls) -> None:
        ...
        cls._base_logger.info(
            f"Class {cls.__name__}: Adding test user to groups [{cls.publisher_wallet_admin_group}, {cls.publisher_wallet_test_users_group}]"
        )
    ...
    def do_on_teardown_class(cls) -> None:
        cls._base_logger.info(
            f"Class {cls.__name__}: Removing test user from groups [{cls.publisher_wallet_admin_group}, {cls.publisher_wallet_test_users_group}]"
        )
