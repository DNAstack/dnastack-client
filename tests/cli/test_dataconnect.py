...
class TestDataConnectCommand(DeprecatedPublisherCliTestCase, DataConnectTestCaseMixin):
    ...
    def __run_test_table_and_search_apis(
        self,
        ...
        table_info = self.simple_invoke(
            "dc", "tables", "get", *self._get_default_parameters(), target_table_info["name"]
        )
        ...
        self.assertEqual(
            sorted(sample_column_names), sorted(selected_column_names), f"Expected columns: {sample_column_names}"
        )
