...
class TestDatasourcesCommand(PublisherCliTestCase):
    ...
    def test_list_datasources_with_type_filter(self):
        ...
        for datasource in filtered_datasources:
            self.assertEqual(
                datasource["type"].upper(), test_type.upper(), f"Expected all datasources to be of type {test_type}"
            )
        ...
        test_cases = [
            {
                ...
                "expected": {"id": "test-id", "name": "test-name", "type": "test-type"},
            },
            {
                "name": "partial datasource",
                "input": {"id": "test-id", "extra_field": "should-not-appear"},
                "expected": {"id": "test-id", "name": None, "type": None},
            },
            {"name": "empty datasource", "input": {}, "expected": {"id": None, "name": None, "type": None}},
        ]
