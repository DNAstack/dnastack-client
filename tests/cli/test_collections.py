...
class TestCollectionsCommand(DeprecatedPublisherCliTestCase):
    ...
    def test_general_flow(self):
        ...
        collections = self.simple_invoke("collections", "list", "--endpoint-id=collection-service")
        ...
        for collection in collections:
            ...
            try:
                ...
                tables = self.simple_invoke(
                    "cs", "tables", "list", "-c", collection_slug_name, "--endpoint-id=collection-service"
                )
                ...
                # JSON version
                items_from_direct_query = self.simple_invoke(
                    "collections",
                    "list-items",
                    "--limit",
                    str(max_size),
                    ...
                    "--endpoint-id=collection-service",
                )
                ...
                items_from_command = self.simple_invoke(
                    "collections",
                    "list-items",
                    "-c",
                    collection_slug_name,
                ...
                common_ids = set([i["id"] for i in items_from_direct_query]).intersection(
                    [i["id"] for i in items_from_command]
                )
                ...
                self._logger.warn(
                    f'Collection "{collection_slug_name}" is not usable for this test. (CLI unexpected termination)'
                )
                last_error = e
    ...
    def test_182678656(self):
        ...
        for c in collections:
            try:
                self.simple_invoke(
                    "collections", "query", "-c", c["slugName"], "SELECT 1", "--endpoint-id=collection-service"
                )
    ...
    def test_182881149(self):
        ...
        with self.assert_exception_raised_in_chain(UnknownCollectionError):
            self.invoke(
                "collections", "query", "-c", f"foobar-{uuid4()}", "SELECT 1", "--endpoint-id=collection-service"
            )
