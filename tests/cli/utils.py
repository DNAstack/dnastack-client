...
def assert_has_property(self: TestCase, obj: dict, attribute: str):
    self.assertTrue(
        attribute in obj,
        msg="obj lacking an attribute. obj: %s, intendedAttribute: %s" % (obj, attribute),
    )
...
def clear_config():
    subprocess.call(
        f"rm {os.getenv('HOME')}/.dnastack/config.yaml && touch {os.getenv('HOME')}/.dnastack/config.yaml", shell=True
    )
...
def use_config_from_file(filename: str):
    try:
        with open(f"{os.getenv('HOME')}/.dnastack/config.yaml", "w") as config_file:
            with open(filename) as config_base:
...
def get_cli_config(runner: CliRunner, key: str, delimiter: str = ".", datatype: type = str):
...
def set_cli_config(runner: CliRunner, key: str, val: Any, delimiter: str = "."):
    result = runner.invoke(dnastack_cli.dnastack, ["config", "set", key, val, "--delimiter", delimiter])
...
# AUTH
def login_with_refresh_token_for_service(runner: CliRunner, service: str, refresh_token: str):
    if service == "dataconnect":
        config_service = "data_connect"
    else:
        config_service = service

    set_cli_config(runner, f"{config_service}.auth.refresh_token", refresh_token, delimiter="|")
