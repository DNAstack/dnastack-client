...
class TestPublisherCommand(PublisherCliTestCase):
    ...
    def setUp(self) -> None:
        ...
        if not any(registry["id"] == collection_service_registry_id for registry in existing_registries):
            self.invoke(
                ...
                "registries",
                "add",
                f"{collection_service_registry_id}",
                f"{self.collection_service_base_url}/service-registry",
            )
        else:
    ...
    def _get_first_collection_with_table(self):
        collections_result = self.simple_invoke("publisher", "collections", "list")
    ...
    def test_collections_list(self):
        collections_result = [
            Collection(**collection) for collection in self.simple_invoke("publisher", "collections", "list")
        ]
    ...
    def test_collections_describe_with_duplicate_ids(self):
        describe_collections = self.simple_invoke(
            "publisher", "collections", "describe", self.collection.id, self.collection.id
        )
    ...
    def test_collections_describe_with_duplicate_results(self):
        describe_collections = self.simple_invoke(
            "publisher", "collections", "describe", self.collection.slugName, self.collection.id
        )
    ...
    def test_collections_create(self):
        ...
        created_collection = Collection(
            **self.simple_invoke(
                ...
                collection_name,
                "--description",
                "Cohort of participants with quality of life assessments",
                "--slug",
                collection_name,
            )
        )
    ...
    def test_collections_query(self):
        ...
        tables_result = self.simple_invoke(
            "publisher", "collections", "tables", "list", "--collection", collection_slug
        )
    ...
    def test_collections_items_list(self):
        ...
        items_result = self.simple_invoke(
            ...
            "list",
            "--collection",
            collection_with_table.slugName,
            "--max-results",
            1,
        )
    ...
    def test_collections_items_add_with_missing_required_fields(self):
        # Missing collection
        self.expect_error_from(
            ["publisher", "collections", "items", "add", "--datasource", "test-datasource", "--files", "file1.txt"],
            r".*Missing option \'--collection\'.*",
        )

        # Missing datasource
        self.expect_error_from(
            ["publisher", "collections", "items", "add", "--collection", "test-collection", "--files", "file1.txt"],
    ...
    def test_collections_items_remove_with_missing_required_fields(self):
        # Missing collection
        self.expect_error_from(
            ["publisher", "collections", "items", "remove", "--datasource", "test-datasource", "--files", "file1.txt"],
            r".*Missing option \'--collection\'.*",
        )

        # Missing datasource
        self.expect_error_from(
            ["publisher", "collections", "items", "remove", "--collection", "test-collection", "--files", "file1.txt"],
            r".*Error: Missing option \'--datasource\'.*",
        )
    ...
    def test_collections_tables_list(self):
        collection_with_table = self._get_first_collection_with_table()
        tables_result = self.simple_invoke(
            "publisher", "collections", "tables", "list", "--collection", collection_with_table.slugName
        )
