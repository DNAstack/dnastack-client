...
from dnastack.client.workbench.samples.models import Instrument, Sample, SampleFile
from dnastack.client.workbench.storage.models import PlatformType, Provider, StorageAccount
...
class TestWorkbenchCommand(WorkbenchCliTestCase):
    ...
    def test_engine_list(self):
        engines_result = [ExecutionEngine(**engine) for engine in self.simple_invoke("workbench", "engines", "list")]
    ...
    def test_engine_describe(self):
        engines_result = [
            ...
            for engine in self.simple_invoke("workbench", "engines", "describe", self.execution_engine.id)
        ]
    ...
    def test_engine_parameters_list(self):
        engine_params_result = [
            EngineParamPreset(**param)
            for param in self.simple_invoke(
    ...
    def test_engine_health_check_list(self):
        engine_health_checks_result = [
            EngineHealthCheck(**health_check)
            for health_check in self.simple_invoke(
    ...
    def test_runs_list_page_and_page_size(self):
        ...
        self.assertNotEqual(
            run_id_on_first_page,
            run_id_on_second_page,
    ...
    def test_runs_list_order_order(self):
        ...
        self.assertNotEqual(
            run_id_from_asc_runs,
            run_id_from_desc_runs,
            f"Expected two different runs when ordered. Found {run_id_from_asc_runs} and {run_id_from_desc_runs}",
        )
    ...
    def test_runs_describe(self):
        runs = self.simple_invoke("workbench", "runs", "list", "--max-results", 2)
        self.assertEqual(len(runs), 2, f"Expected exactly two runs. Found {runs}")
        ...
        def test_single_run():
            described_runs = [
                ...
                for described_run in self.simple_invoke("workbench", "runs", "describe", first_run_id)
            ]
        ...
        def test_multiple_runs():
            described_runs = [
                ExtendedRun(**described_run)
                for described_run in self.simple_invoke("workbench", "runs", "describe", first_run_id, second_run_id)
            ]
        ...
        def test_status():
            described_runs = [
                MinimalExtendedRun(**described_run)
                for described_run in self.simple_invoke("workbench", "runs", "describe", "--status", first_run_id)
            ]
            self.assertEqual(len(described_runs), 1, f"Expected exactly one run. Found {described_runs}")
        ...
        def test_inputs():
            described_runs = [
                MinimalExtendedRunWithInputs(**described_run)
                for described_run in self.simple_invoke("workbench", "runs", "describe", "--inputs", first_run_id)
            ]
            self.assertEqual(len(described_runs), 1, f"Expected exactly one run. Found {described_runs}")
        ...
        def test_outputs():
            described_runs = [
                MinimalExtendedRunWithOutputs(**described_run)
                for described_run in self.simple_invoke("workbench", "runs", "describe", "--outputs", first_run_id)
            ]
            self.assertEqual(len(described_runs), 1, f"Expected exactly one run. Found {described_runs}")
    ...
    def test_runs_cancel(self):
        runs = self.simple_invoke("workbench", "runs", "list", "--max-results", 1)
        self.assertEqual(len(runs), 1, f"Expected exactly one run. Found {runs}")
        first_run_id = ExtendedRunStatus(**runs[0]).run_id

        BatchActionResult(**self.simple_invoke("workbench", "runs", "cancel", first_run_id))
    ...
    def test_runs_delete(self):
        ...
        BatchActionResult(**self.simple_invoke("workbench", "runs", "delete", "--force", last_run_id))
    ...
    def test_runs_submit(self):
        ...
        def test_submit_batch_with_workflow_and_version():
            ...
            submitted_batch = BatchRunResponse(
                **self.simple_invoke(
                    "workbench",
                    "runs",
                    "submit",
            ...
            described_runs = [
                MinimalExtendedRunWithInputs(**described_run)
                for described_run in self.simple_invoke(
                    "workbench", "runs", "describe", "--inputs", submitted_batch.runs[0].run_id
                )
            ]
            self.assertEqual(len(described_runs), 1, f"Expected exactly one run. Found {described_runs}")
            self.assertEqual(
                described_runs[0].inputs,
                {"test.hello.name": "foo"},
        ...
        def test_submit_batch_with_single_embedded_text_params():
            submitted_batch = BatchRunResponse(
                **self.simple_invoke(
                    ...
                    hello_world_workflow_url,
                    "--workflow-params",
                    f"test.hello.name=@{input_text_file}",
                )
            )
            self.assertEqual(len(submitted_batch.runs), 1, "Expected exactly one run to be submitted.")
            described_runs = [
        ...
        def test_submit_batch_with_engine_key_value_param():
            submitted_batch = BatchRunResponse(
                **self.simple_invoke(
                    "workbench", "runs", "submit", "--url", hello_world_workflow_url, "--engine-params", "key=value"
                )
            )
            self.assertEqual(len(submitted_batch.runs), 1, "Expected exactly one run to be submitted.")
            described_runs = [
                ExtendedRun(**described_run)
                for described_run in self.simple_invoke("workbench", "runs", "describe", submitted_batch.runs[0].run_id)
            ]
            self.assertEqual(len(described_runs), 1, f"Expected exactly one run. Found {described_runs}")
            self.assertTrue(
                "key" in described_runs[0].request.workflow_engine_parameters
                and "value" in described_runs[0].request.workflow_engine_parameters["key"],
                "Expected workflow engine params to be exactly the same. "
        ...
        def test_submit_batch_with_engine_preset_param():
            ...
            self.assertEqual(
                described_runs[0].request.workflow_engine_parameters,
        ...
        def test_submit_batch_with_dry_run_option():
            submitted_batch_request = BatchRunRequest(
                **self.simple_invoke(
                    "workbench",
                    "runs",
                    "submit",
                    ...
                    "foo=bar",
                    "--samples",
                    "HG001,HG002,HG003,HG004",
                )
            )
            self.assertEqual(
                len(submitted_batch_request.run_requests), 1, "Expected exactly one run request submitted."
            )
            self.assertEqual(
                submitted_batch_request.run_requests[0].workflow_params,
                {"test.hello.name": "foo"},
                "Expected workflow params to be the same.",
            )
            self.assertEqual(
                submitted_batch_request.workflow_url, hello_world_workflow_url, "Expected workflow url to be the same."
            )
            self.assertEqual(submitted_batch_request.default_tags, {"foo": "bar"}, "Expected tags to be the same.")
            self.assertEqual(
                submitted_batch_request.samples,
                [Sample(id="HG001"), Sample(id="HG002"), Sample(id="HG003"), Sample(id="HG004")],
                "Expected created samples classes with the same ids",
            )
    ...
    def test_run_events_list(self):
        ...
        events_result = [
            RunEvent(**run_event)
            for run_event in self.simple_invoke("workbench", "runs", "events", "list", "--run-id", first_run_id)
        ]
    ...
    def test_samples_list_and_describe(self):
        ...
        sample = Sample(**self.simple_invoke("workbench", "samples", "describe", samples[0].id))
    ...
    def _wait_for_samples(self):
        ...
        while True:
            samples = [Sample(**sample) for sample in self.simple_invoke("workbench", "samples", "list")]
            if samples:
                break
    ...
    def test_samples_files_list(self):
        ...
        sample_files = [
            ...
            for sample_file in self.simple_invoke("workbench", "samples", "files", "list", "--sample", sample.id)
        ]
        ...
        sample_files = [
            SampleFile(**sample_file)
            for sample_file in self.simple_invoke(
                "workbench",
                "samples",
                "files",
                "list",
    ...
    def test_update_aws_storage_account(self):
        ...
        updated_storage_account = StorageAccount(
            **self.simple_invoke(
                "workbench",
                "storage",
                "update",
    ...
    def test_storage_add_azure_with_conflicting_auth_methods(self):
        self.expect_error_from(
            [
                ...
                "azure",
                "--name",
                "Test GCP Storage Account with Access Key",
    ...
    def test_storage_list(self):
        ...
        storage_accounts = [
            StorageAccount(**storage_account) for storage_account in self.simple_invoke("workbench", "storage", "list")
        ]
    ...
    def test_storage_describe(self):
        ...
        storage_accounts = [
            ...
            for storage_account in self.simple_invoke("workbench", "storage", "describe", created_storage_account.id)
        ]
    ...
    def test_storage_delete(self):
        ...
        output = self.simple_invoke(
            "workbench", "storage", "delete", created_storage_account.id, "--force", parse_output=False
        )
        ...
        result = self.invoke("workbench", "storage", "describe", created_storage_account.id, bypass_error=True)
    ...
    def test_workflows_list(self):
        result = [Workflow(**workflow) for workflow in self.simple_invoke("workbench", "workflows", "list")]
        self.assert_not_empty(result, "Expected at least one workflows.")

        def test_source():
            result = [
                Workflow(**workflow)
                for workflow in self.simple_invoke("workbench", "workflows", "list", "--source", "DOCKSTORE")
            ]
            self.assertGreater(len(result), 0, "Expected at least one workflow.")
            self.assertTrue(
                all(workflow.source == "DOCKSTORE" for workflow in result), "Expected all workflows to be DOCKSTORE."
            )

            result = [
                Workflow(**workflow)
                for workflow in self.simple_invoke("workbench", "workflows", "list", "--source", "PRIVATE")
            ]
            if len(result) == 0:
                ...
                self.assertGreater(len(result), 0, "Expected at least one workflow.")
                self.assertTrue(
                    all(workflow.source == "PRIVATE" for workflow in result), "Expected all workflows to be PRIVATE."
                )
    ...
    def test_workflows_describe(self):
        ...
        def test_single_workflow():
            described_workflow = [
                ...
                for described_workflow in self.simple_invoke("workbench", "workflows", "describe", first_workflow_id)
            ]
        ...
        def test_multiple_workflows():
            described_workflows = [
                ...
                for described_workflow in self.simple_invoke(
                    "workbench", "workflows", "describe", first_workflow_id, second_workflow_id
                )
            ]
    ...
    def test_workflows_create_and_add_version(self):
        ...
        def test_create_workflow():
            created_workflow_from_file = Workflow(
                **self.simple_invoke(
                    "workbench",
                    "workflows",
                    "create",
            ...
            created_workflow_from_zip = Workflow(
                **self.simple_invoke(
                    "workbench",
                    "workflows",
                    "create",
        ...
        def test_edit_workflow():
            ...
            edited_workflow = Workflow(
                **self.simple_invoke(
                    "workbench", "workflows", "update", "--description", "@description.md", created_workflow.internalId
                )
            )

            self.assertTrue("TITLE" in edited_workflow.description and "DESCRIPTION" in edited_workflow.description)
        ...
        def test_add_version():
            ...
            described_workflows = [
                Workflow(**described_workflow)
                for described_workflow in self.simple_invoke(
                    "workbench", "workflows", "describe", created_workflow.internalId
                )
            ]
            self.assertTrue(
                any(
                    described_workflow_version.id == created_workflow_version.id
        ...
        def test_edit_version():
            edited_workflow_version = WorkflowVersion(
                **self.simple_invoke(
                    "workbench",
                    "workflows",
                    "versions",
        ...
        def test_delete_version():
            ...
            versions = [
                ...
                for workflow_version in self.simple_invoke(
                    "workbench",
                    "workflows",
                    "versions",
                    "list",
        ...
        def test_delete_workflow():
            ...
            output = self.simple_invoke(
                "workbench", "workflows", "delete", "--force", workflow_to_delete.internalId, parse_output=False
            )

            self.assertTrue("Deleted..." in output)

            workflows = [Workflow(**workflow) for workflow in self.simple_invoke("workbench", "workflows", "list")]
            ...
            result = self.invoke("workbench", "workflows", "describe", workflow_to_delete.internalId, bypass_error=True)
    ...
    def test_workflow_version_list(self):
        workflow_result = [Workflow(**workflow) for workflow in self.simple_invoke("workbench", "workflows", "list")]
    ...
    def test_workflow_version_describe(self):
        ...
        workflow = [Workflow(**w) for w in self.simple_invoke("workbench", "workflows", "describe", workflow_id)][0]

        result = [
            WorkflowVersion(**workflow_version)
            for workflow_version in self.simple_invoke(
                "workbench", "workflows", "versions", "describe", "--workflow", workflow_id, workflow.versions[0].id
            )
        ]
    ...
    def test_create_workflow_defaults(self):
        ...
        ## JSON inputs
        created_default = WorkflowDefaults(
            **self.simple_invoke(
                "workbench",
                "workflows",
                "versions",
    ...
    def test_list_workflow_defaults(self):
        ...
        list_result = [
            WorkflowDefaults(**workflow_default)
            for workflow_default in self.simple_invoke(
                "workbench",
                "workflows",
                "versions",
                "defaults",
    ...
    def test_describe_workflow_defaults(self):
        ...
        describe_result = WorkflowDefaults(
            **self.simple_invoke(
                "workbench",
                "workflows",
                "versions",
    ...
    def test_update_workflow_defaults(self):
        ...
        updated_default = WorkflowDefaults(
            **self.simple_invoke(
                "workbench",
                "workflows",
                "versions",
    ...
    def test_workflow_transformation_create_with_file(self):
        ...
        self.assertMultiLineEqual(
            workflow_transformation.script.replace(" ", "").replace("\n", ""), "(context)=>{return{'baz':'waz'}}"
        )
    ...
    def test_workflow_transformation_list(self):
        ...
        transformations = [
            WorkflowTransformation(**transformation)
            for transformation in self.simple_invoke(
                "workbench",
                "workflows",
                "versions",
                "transformations",
        ...
        self.assert_not_empty(
            transformations, f"Expected at least one workflow transformation. Found {transformations}"
        )
    ...
    def test_workflow_transformation_delete(self):
        ...
        workflow = self._create_workflow()
        workflow_version = self._create_workflow_version(workflow.internalId, "v1")
        workflow_transformation_to_be_deleted = self._create_workflow_transformation(
            workflow.internalId, workflow_version.id
        )
    ...
    def test_instruments_list(self):
        ...
        self._wait()
        instruments = [
            Instrument(**instrument) for instrument in self.simple_invoke("workbench", "instruments", "list")
        ]
