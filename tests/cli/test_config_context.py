...
class TestCommand(DeprecatedPublisherCliTestCase):
    ...
    def test_use_command_with_auth_enabled_on_context_switch(self):
        ...
        self.assertGreater(
            len(config.contexts[config.current_context].endpoints), 0, "There should be at least one endpoints."
        )
        ...
        handle_device_code_flow(
            ["python", "-m", "dnastack", "use", secondary_context_host], self._states["email"], self._states["token"]
        )
    ...
    def test_use_command_with_auth_disabled_on_context_switch(self):
        ...
        self.assertGreaterEqual(len(self._get_context_names()), 1, "There should be at least ONE context.")
        self.assertIn(self._explorer_hostname, self._get_context_names(), "The target context should be listed")
        config = self._load_configuration()
    ...
    def test_use_command_with_exact_url(self):
        ...
        self.invoke(
            "use",
            urljoin(self._explorer_base_url, "/api/service-registry"),
            "--no-auth",
            timeout=30,
            bypass_error=False,
        )
    ...
    def test_use_command_error_recovery(self):
        ...
        for request_path in ("/foo/api", "/bar/api/service-registry"):
            self._temporarily_remove_existing_config()
            result = self.invoke(
                "use", urljoin(self._explorer_base_url, request_path), "--no-auth", timeout=30, bypass_error=True
            )
            self.assertNotEqual(
                result.exit_code,
                0,
    ...
    def test_use_command_with_unusable_registry(self):
        ...
        self.assertNotIn(
            "localhost",
            self._get_context_names(),
            "localhost shouldn't be a context since it is a invalid registry url",
        )
