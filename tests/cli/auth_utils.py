...
try:
    ...
    stderr.write(
        'WARNING: The package "selenium" is not detected. This may break any tests that require selenium, '
...
def handle_device_code_flow(cmd: List[str], email: str, token: str) -> str:
    ...
    process_env = {e_k: e_v for e_k, e_v in os.environ.items() if e_k != "DNASTACK_DEBUG"}
...
def _login_via_personal_access_token(driver: WebDriver, email: str, token: str, device_code_url: str):
    ...
    WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.XPATH, "//a[contains(@href, 'login')]")))
...
def confirm_device_code(device_code_url: str, email: str, token: str):
    ...
    try:
        ...
        WebDriverWait(driver, 10).until(
            EC.visibility_of_element_located((By.ID, "authorization")), message="Expecting authorization page"
        )
        # Fill user code in device authorization page
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, "//input[@name='user_code']"))).send_keys(
            user_code
        )
        # Click on continue button
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID, "continue-btn"))).click()
        # Assert device confirmation page is loaded
        try:
            WebDriverWait(driver, 10).until(
                EC.visibility_of_element_located((By.ID, "confirmation")), message="Expecting device confirmation page"
            )
        ...
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID, "allow-btn"))).click()
