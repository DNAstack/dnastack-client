...
class TestDrsCommand(DeprecatedPublisherCliTestCase):
    ...
    def setUp(self):
        ...
        if not self.primary_factory:
            self.primary_factory = (
                use(self.explorer_urls[1], no_auth=True) if self._test_via_explorer else self.get_factory()
            )
    ...
    def test_download_files_with_cli_arguments(self):
        self.retry_if_fail(
            self._test_download_files_with_cli_arguments, intermediate_cleanup=lambda: self._clear_temp_files()
        )
    ...
    def test_download_files_with_cli_arguments_no_output_dir(self):
        self.retry_if_fail(
            self._test_download_files_with_cli_arguments_no_output_dir,
    ...
    def test_download_files_with_input_file(self):
        self.retry_if_fail(
            self._test_download_files_with_input_file,
            intermediate_cleanup=lambda: self._clear_temp_files(),
            max_run_count=0,
        )
