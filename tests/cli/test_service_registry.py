...
class TestCliServiceRegistry(DeprecatedPublisherCliTestCase):
    ...
    def test_happy_path(self):
        ...
        self.invoke(
            "config", "registries", "add", registry_1_id, urljoin(self._collection_service_url, "/service-registry/")
        )
        ...
        self.assert_not_empty(
            [e for e in post_adding_list if e["id"] == "test-collection-service"],
        ...
        self.invoke(
            "config", "reg", "add", registry_2_id, "https://collection-service.publisher.dnastack.com/service-registry/"
        )
        ...
        self._assert_if_endpoint_lists_are_identical(
            gcp_prod_endpoints, self.simple_invoke("config", "reg", "list-endpoints", registry_2_id)
        )
        ...
        self.assertEqual(
            initial_list, post_unsync_list, "The current list of endpoints should be the same as the initial list."
        )
    ...
    def test_user_cannot_add_registry_with_known_id(self):
        ...
        self.expect_error_from(
            ["config", "reg", "add", "panda-service", "https://panda.faux.dnastack.com"],
    ...
    def test_user_cannot_add_registry_with_known_registry_url(self):
        ...
        self.expect_error_from(
            ["config", "reg", "add", "test-registry-002", urljoin(self._collection_service_url, "/service-registry/")],
            error_regex="^EndpointAlreadyExists:",
        )

    def test_user_cannot_sync_with_unknown_registry(self):
        self.expect_error_from(["config", "registries", "sync", "lala"], error_regex="^RegistryNotFound:")

    def _assert_if_endpoint_lists_are_identical(
        self, expected_list: Iterable[Dict[str, Any]], given_list: Iterable[Dict[str, Any]]
    ):
        def _simplify(service: Dict[str, Any]) -> Dict[str, Any]:
            return {
                k: v for k, v in service.items() if k in ["fallback_authentications", "authentication", "type", "url"]
            }

        self.assertEqual(
            SimpleStream(expected_list).map(_simplify).to_list(),
