...
class TestExplorerFocused(unittest.TestCase):
    ...
    def test_utils_format_question_parameters_detailed(self):
        ...
        param1 = QuestionParam(id="1", name="param1", label="Param 1", inputType="string", values="val1\nval2")
        result = format_question_parameters([param1])
        self.assertIn("param1", result)

        # Test with parameter having default value
        param2 = QuestionParam(id="2", name="param2", label="Param 2", inputType="string", defaultValue="default_val")
        result = format_question_parameters([param2])
        self.assertIn("default_val", result)

        # Test with parameter having no description
        param3 = QuestionParam(id="3", name="param3", label="Param 3", inputType="string")
    ...
    def test_utils_format_question_collections_detailed(self):
        ...
        collection = QuestionCollection(id="c1", name="Collection One", slug="collection-one", questionId="q1")
    ...
    def test_utils_validate_question_parameters_errors(self):
        ...
        question = FederatedQuestion(
            ...
            name="Test",
            description="Test",
            params=[QuestionParam(id="1", name="required_param", label="Required", inputType="string", required=True)],
            collections=[],
        )
    ...
    def test_tables_format_question_list_with_real_data(self):
        ...
        question = FederatedQuestion(
            ...
            params=[
                QuestionParam(
                    id="1", name="required_param", label="Required Parameter", inputType="string", required=True
                ),
                QuestionParam(
                    id="2", name="optional_param", label="Optional Parameter", inputType="integer", required=False
                ),
            ],
            collections=[QuestionCollection(id="c1", name="Collection 1", slug="collection-1", questionId="q1")],
        )
    ...
    def test_tables_format_question_detail_with_realistic_data(self):
        ...
        question = FederatedQuestion(
            ...
            collections=[
                QuestionCollection(id="c1", name="Test Collection", slug="test-collection", questionId="complex_q")
            ],
        )
    ...
    def test_client_service_types_comprehensive(self):
        ...
        from dnastack.client.explorer.client import EXPLORER_SERVICE_TYPE_V1_0, ExplorerClient
    ...
    def test_model_dict_conversion(self):
        ...
        param = QuestionParam(id="1", name="test_param", label="Test Parameter", inputType="string", required=True)

        collection = QuestionCollection(id="c1", name="Test Collection", slug="test-collection", questionId="q1")

        question = FederatedQuestion(
            id="q1", name="Test Question", description="Test description", params=[param], collections=[collection]
        )
