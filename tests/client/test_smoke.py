...
from dnastack.client.data_connect import TableInfo, TableNotFoundError
...
class TestSmokeWithAuthentication(DeprecatedBasePublisherTestCase):
    ...
    def test_demo(self):
        ...
        endpoint = ServiceEndpoint(
            url=urljoin(self._explorer_base_url, "/api/"), type=EXPLORER_COLLECTION_SERVICE_TYPE_V1_0
        )
        ...
        for collection_index in range(len(collections)):
            ...
            try:
                ...
                self._logger.debug(
                    f"Checking if this collection is good for testing with table(s) {[t.name for t in tables]}"
                )
                ...
                for listed_table in tables:
                    ...
                    try:
                        self.assert_not_empty(
                            self._get_subset_of(table.data, 100), "Failed to fetch the data from table."
                        )
                    except TableNotFoundError as e:
                        self._logger.warning(
                            f"T/{target_table_name}: Encountered unexpected error while interacting with /table/{target_table_name}/data"
                        )
                ...
                while table_index < len(queried_tables):
                    ...
                    try:
                        ...
                        self._logger.warning(
                            f"T/{target_table_name}: Encountered unexpected error while interacting with /search on {target_table_name}"
                        )
                        self._logger.warning(f"T/{target_table_name}: {type(e).__module__}.{type(e).__name__}: {e}")
                ...
                if table_index == len(queried_tables):
                    raise RuntimeError(
                        f"Cannot test as there is no data in any of {', '.join([t['qualified_table_name'] for t in queried_tables])}"
                    )
                ...
                self._logger.warning(
                    f"C/{target_collection.slugName}: Reason: {type(e).__module__}.{type(e).__name__}: {e}"
                )
    ...
    def _get_subset_of(
        self, iterator: Iterator[Dict[str, Any]], max_size: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        rows = []
