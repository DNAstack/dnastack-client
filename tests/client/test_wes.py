...
from dnastack.alpha.client.wes.client import STANDARD_WES_TYPE_V1_0, Run, RunRequest, WesClient
from dnastack.common.environments import EnvironmentVariableRequired, env, flag
...
class TestClient(DeprecatedBasePublisherTestCase):
    ...
    def setUp(self):
        ...
        try:
            self._endpoint_uri = env(
                "E2E_WES_ENDPOINT_URI",
                required=False,
                default="https://workspaces-wes.alpha.dnastack.com/ga4gh/wes/v1/",
            )
            # FIXME Rename E2E_STAGING_CLIENT_ID to E2E_WES_CLIENT_ID
            self._client_id = env(
                "E2E_STAGING_CLIENT_ID",
                description="OAuth Client ID for WES E2E test",
                required=False,
                default="dnastack-client-library-testing",
            )
            # FIXME Rename E2E_STAGING_CLIENT_SECRET to E2E_WES_CLIENT_SECRET
            self._client_secret = env(
                "E2E_STAGING_CLIENT_SECRET", description="OAuth Client Secret for WES E2E test", required=False
            )
            # FIXME Rename E2E_STAGING_TOKEN_URI to E2E_WES_TOKEN_URI
            self._token_endpoint = env(
                "E2E_STAGING_TOKEN_URI", required=False, default="https://wallet.staging.dnastack.com/oauth/token"
            )
            self._resource_url = env(
                "E2E_WES_OAUTH_RESOURCE_URI", required=False, default="https://workspaces-wes.alpha.dnastack.com/"
            )
        ...
        try:
            ...
            for i in range(len(final_nodes) - len(starting_nodes)):
                ...
                detected_files.extend(
                    [os.path.join(node_base_path, fn) for fn in os.listdir(node_base_path) if fn[0] != "."]
                )
            raise RuntimeError(
                f"File not found at {file_path[len(base_path) :]} (exists: {[fn[len(base_path) :] for fn in sorted(detected_files)]})"
            )
