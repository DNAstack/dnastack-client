...
class TestStress(TestCase):
    ...
    def test_issue_180415300(self):
        ...
        """
        client = CollectionServiceClient.make(
            ServiceEndpoint(
                adapter_type="collections", url="https://explorer.beta.rc.dnastack.com/api/", mode="explorer"
            )
        )
        ...
        for row in sub_client.query("SELECT * FROM collections.ncbi_sra.public_variants LIMIT 1500000"):
            ...
            if row_count % 100000 == 0:
                ...
                self._logger.debug(
                    f"Receiving {row_count} rows (mem diff: {self._format_readable_byte_size(current_memory_usage - initial_memory_usage)})..."
                )
            self.assertGreater(len(row.keys()), 0)
        self._logger.auth_info(
            f"Received {row_count} rows (mem diff: {self._format_readable_byte_size(self._get_current_memory_usage() - initial_memory_usage)})..."
        )
