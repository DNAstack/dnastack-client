...
from dnastack.client.collections.client import Collection, CollectionServiceClient, UnknownCollectionError
...
class TestCollectionsClient(DeprecatedBasePublisherTestCase, DataConnectTestCaseMixin):
    ...
    def test_auth_client_interacts_with_collection_api(self):
        ...
        assert collection_client is not None, (
            f"The collection service client is not available. ({self.get_factory().all()})"
        )
