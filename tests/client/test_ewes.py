...
class TestClient(BaseWorkbenchTestCase):
    ...
    def test_submit_run(self):
        ...
        self.assertTrue(
            submitted_run.state in expected_states,
            f"Expected any state from {expected_states}. Instead found {submitted_run.state}.",
        )
    ...
    def test_submit_batch(self):
        ...
        self.assertTrue(
            submitted_batch.runs[0].state in expected_states,
    ...
    def test_get_run(self):
        ...
        self.assertEqual(
            run.request.workflow_params.get("test.hello.name"), "foo", "Expected workflow param to be equal to 'foo'."
        )
    ...
    def test_list_runs(self):
        ...
        runs = list(
            ewes_client.list_runs(
                list_options=ExtendedRunListOptions(engine_id=self.execution_engine.id), max_results=None
            )
        )
        self.assertGreater(len(runs), 1, "Expected at least two runs.")

        runs = list(ewes_client.list_runs(list_options=ExtendedRunListOptions(engine_id="foo"), max_results=None))
