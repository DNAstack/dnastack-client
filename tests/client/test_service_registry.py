...
from dnastack.client.service_registry.client import Service, ServiceRegistry
from dnastack.client.service_registry.factory import (
    ClientFactory,
    RegisteredServiceInfo,
    UnregisteredServiceEndpointError,
)
from dnastack.client.service_registry.models import Organization, ServiceType
from dnastack.common.environments import env
from dnastack.common.model_mixin import JsonModelMixin
from tests.exam_helper import DeprecatedBasePublisherTestCase, device_code_endpoint, token_endpoint

SERVICE_REGISTRY_URL = env(
    "E2E_SERVICE_REGISTRY_URL",
    required=False,
...
REGISTERED_COLLECTION_SERVICE_URL = env(
    "E2E_REGISTERED_COLLECTION_SERVICE_URL",
    required=False,
...
class TestClientFactoryUnit(DeprecatedBasePublisherTestCase):
    mock_service_type_1 = ServiceType(group="com.dnastack", artifact="panda", version="1.2.3")

    mock_service_type_1_older = ServiceType(group="com.dnastack", artifact="panda", version="1.0.1")

    mock_service_type_2 = ServiceType(group="com.dnastack", artifact="x-ray", version="5.7.11")

    mock_service_type_3 = ServiceType(group="com.dnastack", artifact="collection-service", version="1.0.0")

    mock_org = Organization(name="dnastack", url="https://dnastack.com")

    mock_service_1 = Service(
        id=str(uuid4()),
    ...
    mock_service_4_restricted = Service(
        id=str(uuid4()),
        authentication=[
            dict(
    ...
    def test_find_services(self):
        ...
        results = self.drain_iterable(
            factory.find_services(exact_match=True, types=[self.mock_service_type_1, self.mock_service_type_1_older])
        )
        self.assertEqual(len(results), 2)

        # Search combo: loosely match, types (found: 3)
        results = self.drain_iterable(
            factory.find_services(exact_match=False, types=[self.mock_service_type_1, self.mock_service_type_2])
        )
        self.assertEqual(len(results), 3)

        # Search combo: exact match, url (found: 1)
        results = self.drain_iterable(factory.find_services(exact_match=True, url="https://foo.io/api/"))
        self.assertEqual(len(results), 2)
        ...
        results = self.drain_iterable(factory.find_services(exact_match=False, url="https://foo.io/api"))
        self.assertEqual(len(results), 2)

        # Search combo: loosely match, types, incomplete but identical url (found: 2)
        results = self.drain_iterable(
            factory.find_services(
                exact_match=False, types=[self.mock_service_type_1, self.mock_service_type_2], url="https://foo.io/api"
            )
        )
        self.assertEqual(len(results), 2)
        ...
        results = self.drain_iterable(
            factory.find_services(
                exact_match=True,
                types=[self.mock_service_type_1, self.mock_service_type_1_older],
                url="https://foo.io/api/",
            )
        )
    ...
    def test_all_service_infos(self):
            ...
            [
                {
                    ...
                    "organization": {"name": "DNAStack", "url": "https://www.dnastack.com"},
                    "contactUrl": None,
                ...
                },
                {
                    ...
                    "type": {"group": "org.ga4gh", "artifact": "drs", "version": "1.1.0"},
                ...
                },
                {
                    ...
                    "version": "1.0-1028-g9ffdd94",
                    "url": urljoin(
                        self._explorer_base_url, "/api/collection/cdc-wastewater-surveillance/data-connect/"
                    ),
                    "authentication": [
                        {
                            "accessTokenUrl": token_endpoint,
                ...
                },
                {
                    ...
                    "version": "1.0-1028-g9ffdd94",
                    "url": urljoin(
                        self._explorer_base_url, "/api/collection/covid-19-global-cases-and-deaths/data-connect/"
                    ),
                    "authentication": [
                        {
                            "accessTokenUrl": token_endpoint,
                ...
                },
                {
                    ...
                    "version": "1.0-1028-g9ffdd94",
                    "url": urljoin(
                        self._explorer_base_url, "/api/collection/covid-19-global-data-repository/data-connect/"
                    ),
                    "authentication": [
                        {
                            "accessTokenUrl": token_endpoint,
                ...
                },
                {
                    ...
                    "version": "1.0-1028-g9ffdd94",
                    "url": urljoin(
                        self._explorer_base_url, "/api/collection/covid-19-us-data-repository/data-connect/"
                    ),
                    "authentication": [
                        {
                            "accessTokenUrl": token_endpoint,
        ...
        self.assertEqual(
            f"{self._explorer_base_url} {urljoin(self._explorer_base_url, '/ga4gh/drs/')}", first_auth_info["resource"]
        )
    ...
    def _fetch_entries(self, sample_json_response: str) -> List[RegisteredServiceInfo]:
        ...
        mock_registry.list_services.return_value = [Service(**raw_entry) for raw_entry in sample_response]
...
class TestClientFactoryEndToEnd(DeprecatedBasePublisherTestCase):
    ...
    def test_find_services(self):
        ...
        exact_match_result_count = len(
            self.drain_iterable(
                factory.find_services(
                    exact_match=True, types=collection_service_types, url=REGISTERED_COLLECTION_SERVICE_URL
                )
            )
        )
