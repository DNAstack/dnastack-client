    ...
    def generate_markdown(self, include_module_name=True, as_class_member=False, is_class_static_member=False) -> str:
        if self.callable:
            signature = (
                ...
                + " def "
                + generate_signature_string(self, include_module_name, as_class_member)
            ).strip()
        else:
            signature = self.name + (f": {self.return_annotation}" if self.return_annotation else "")
        lines = [
    ...
    def generate_markdown(self) -> str:
        ...
        if self.methods:
            ...
            for method_doc in self.methods:
                ...
                method_markdown = method_doc.generate_markdown(
                    as_class_member=True, is_class_static_member=not is_object_method
                )
...
def extract_doc_from(ref):
    if inspect.isclass(ref):
        cls_doc = ClassDoc(
            module_name=ref.__module__ if hasattr(ref, "__module__") else None,
            name=ref.__name__,
