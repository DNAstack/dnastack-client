...
from dnastack.client.workbench.samples.models import (
    InstrumentListOptions,
    InstrumentListResponse,
    Sample,
    SampleFileListResponse,
    SampleFilesListOptions,
    SampleListOptions,
    SampleListResponse,
)
...
class SamplesClient(BaseWorkbenchClient):
    ...
    def list_samples(
        self, list_options: Optional[SampleListOptions] = None, max_results: int = None, trace: Optional[Span] = None
    ) -> Iterator[Sample]:
        trace = trace or Span(origin=self)
        return ResultIterator(
            SampleListResultLoader(
    ...
    def get_sample(self, sample_id: str, trace: Optional[Span] = None) -> Sample:
        ...
        with self.create_http_session() as session:
            response = session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/samples/{sample_id}"), trace_context=trace
            )
