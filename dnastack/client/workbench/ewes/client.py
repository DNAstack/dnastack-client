...
from dnastack.client.workbench.ewes.models import (
    ...
    BatchRunResponse,
    EngineHealthCheck,
    EngineHealthCheckListOptions,
    EngineHealthCheckListResponse,
    EngineParamPreset,
    EngineParamPresetListOptions,
    EngineParamPresetListResponse,
    ExecutionEngine,
...
class EngineParamPresetListResultLoader(WorkbenchResultLoader):
    def __init(
        ...
        super().__init__(service_url=service_url, http_session=http_session, list_options=list_options, trace=trace)
...
class EWesClient(BaseWorkbenchClient):
    ...
    def get_service_info(self, trace: Optional[Span] = None) -> WesServiceInfo:
        ...
        with self.create_http_session() as session:
            response = session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/service-info"), trace_context=trace
            )
    ...
    def get_status(self, run_id: str, trace: Optional[Span] = None) -> MinimalExtendedRun:
        ...
        with self.create_http_session() as session:
            response = session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/runs/{run_id}/status"), trace_context=trace
            )
    ...
    def get_run(self, run_id: str, include_tasks: bool = False, trace: Optional[Span] = None) -> ExtendedRun:
        ...
        with self.create_http_session() as session:
            response = session.get(
                urljoin(
                    self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/runs/{run_id}?exclude_tasks={not include_tasks}"
                ),
                trace_context=trace,
            )
    ...
    def cancel_run(self, run_id: str, trace: Optional[Span] = None) -> Union[RunId, WorkbenchApiError]:
        with self.create_http_session() as session:
            response = session.post(
                urljoin(self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/runs/{run_id}/cancel"), trace_context=trace
            )
    ...
    def cancel_runs(self, run_ids: List[str], trace: Optional[Span] = None) -> BatchActionResult:
        ...
        with self.create_http_session() as session:
            response = session.post(
                urljoin(self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/runs/cancel"),
                json=run_ids,
                trace_context=trace,
            )
    ...
    def delete_runs(self, run_ids: List[str], trace: Optional[Span] = None) -> BatchActionResult:
        ...
        with self.create_http_session() as session:
            response = session.post(
                urljoin(self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/runs/delete"),
                json=run_ids,
                trace_context=trace,
            )
            return BatchActionResult(**response.json())
    ...
    def submit_run(self, data: ExtendedRunRequest, trace: Optional[Span] = None) -> MinimalExtendedRun:
        ...
        with self.create_http_session() as session:
            response = session.post(
                urljoin(self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/runs"), json=data.dict(), trace_context=trace
            )
        return MinimalExtendedRun(**response.json())
    ...
    def submit_batch(self, data: BatchRunRequest, trace: Optional[Span] = None) -> BatchRunResponse:
        with self.create_http_session() as session:
            response = session.post(
                urljoin(self.endpoint.url, f"{self.namespace}/ga4gh/wes/v1/batch"),
                json=data.dict(),
                trace_context=trace,
            )
    ...
    def stream_log_url(
        self, log_url: str, max_bytes: Optional[int], offset: Optional[int], trace: Optional[Span] = None
    ) -> Iterable[bytes]:
        trace = trace or Span()
        ...
        with self.create_http_session() as session:
            with session.get(
                urljoin(self.endpoint.url, log_url), params=params, stream=True, trace_context=trace
            ) as response:
    ...
    def list_engines(
        self, list_options: ExecutionEngineListOptions, max_results: int = None, trace: Optional[Span] = None
    ) -> Iterable[ExecutionEngine]:
        trace = trace or Span(origin=self)
        return ResultIterator(
            EngineListResultLoader(
    ...
    def get_engine(self, engine_id: str, trace: Optional[Span] = None) -> ExecutionEngine:
        with self.create_http_session() as session:
            response = session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/engines/{engine_id}"), trace_context=trace
            )
            return ExecutionEngine(**response.json())

    def list_engine_param_presets(
        self, engine_id: str, list_options: EngineParamPresetListOptions, trace: Optional[Span] = None
    ) -> Iterable[EngineParamPreset]:
        trace = trace or Span(origin=self)
        return ResultIterator(
            EngineParamPresetListResultLoader(
    ...
    def get_engine_param_preset(
        self, engine_id: str, param_preset_id: str, trace: Optional[Span] = None
    ) -> EngineParamPreset:
        with self.create_http_session() as session:
            response = session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/engines/{engine_id}/param-presets/{param_preset_id}"),
                trace_context=trace,
            )
            return EngineParamPreset(**response.json())

    def list_engine_health_checks(
        self,
        engine_id: str,
        list_options: EngineHealthCheckListOptions,
        max_results: int = None,
        trace: Optional[Span] = None,
    ) -> Iterable[EngineHealthCheck]:
        trace = trace or Span(origin=self)
        return ResultIterator(
            EngineHealthCheckListResultLoader(
