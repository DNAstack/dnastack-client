...
from dnastack.client.workbench.workflow.models import (
    ...
    WorkflowDefaultsListOptions,
    WorkflowDefaultsListResponse,
    WorkflowDefaultsSelector,
    ...
    WorkflowTransformationListOptions,
    WorkflowTransformationListResponse,
    WorkflowVersion,
...
class WorkflowClient(BaseWorkbenchClient):
    ...
    def list_workflows(
        self, list_options: Optional[WorkflowListOptions] = None, max_results: int = None
    ) -> Iterator[Workflow]:
        return ResultIterator(
            WorkflowsListResultLoader(
    ...
    def list_workflow_versions(
        self, workflow_id: str, list_options: Optional[WorkflowVersionListOptions] = None, max_results: int = None
    ) -> Iterator[WorkflowVersion]:
        return ResultIterator(
            WorkflowVersionsListResultLoader(
    ...
    def get_workflow_version(
        self, workflow_id: str, version_id: str, include_deleted: Optional[bool] = False
    ) -> WorkflowVersion:
        with self.create_http_session() as session:
            response = session.get(
                urljoin(
    ...
    def get_workflow_files(self, workflow_id: str, version_id: str) -> List[WorkflowFile]:
        with self.create_http_session() as session:
            response = session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/workflows/{workflow_id}/versions/{version_id}/files")
            )
            return [WorkflowFile(**item) for item in response.json()]
    ...
    def get_workflow_descriptor(self, workflow_id: str, version_id: str) -> WorkflowFile:
        with self.create_http_session() as session:
            with session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/workflows/{workflow_id}/versions/{version_id}/descriptor")
            ) as response:
    ...
    def create_version(
        self, workflow_id: str, workflow_version_create_request: WorkflowVersionCreate
    ) -> WorkflowVersion:
        with self.create_http_session() as session:
            data = {
    ...
    def delete_workflow(self, workflow_id: str, etag: str):
        with self.create_http_session() as session:
            session.delete(
                urljoin(self.endpoint.url, f"{self.namespace}/workflows/{workflow_id}"), headers={"If-Match": etag}
            )
    ...
    def update_workflow_version(
        self, workflow_id: str, version_id: str, etag: str, updates: List[JsonPatch]
    ) -> WorkflowVersion:
        with self.create_http_session() as session:
            updates = [update.dict() for update in updates]
            response = session.json_patch(
    ...
    def resolve_workflow(self, workflow_url: str) -> ResolvedWorkflow:
        with self.create_http_session() as session:
            response = session.get(
                urljoin(self.endpoint.url, f"{self.namespace}/workflows/resolve"), params={"workflow-url": workflow_url}
            )
            return ResolvedWorkflow(**response.json())

    def resolve_workflow_defaults(
        self, workflow_id: str, workflow_version_id: str, selector: WorkflowDefaultsSelector
    ) -> WorkflowDefaults:
        with self.create_http_session() as session:
            response = session.post(
                urljoin(
                ...
                params={"engine": selector.engine, "provider": selector.provider, "region": selector.region},
            )
            return WorkflowDefaults(**response.json())

    def list_workflow_defaults(
        self, workflow_id: str, version_id: str, max_results: int, list_options: WorkflowDefaultsListOptions
    ) -> Iterator[WorkflowDefaults]:
        return ResultIterator(
            WorkflowDefaultsListResultLoader(
                service_url=urljoin(
                    self.endpoint.url, f"{self.namespace}/workflows/{workflow_id}/versions/{version_id}/defaults"
                ),
                http_session=self.create_http_session(),
                list_options=list_options,
                trace=None,
    ...
    def create_workflow_defaults(
        self, workflow_id: str, version_id: str, workflow_defaults: WorkflowDefaultsCreateRequest
    ) -> WorkflowDefaults:
        with self.create_http_session() as session:
            response = session.post(
                urljoin(self.endpoint.url, f"{self.namespace}/workflows/{workflow_id}/versions/{version_id}/defaults"),
    ...
    def list_workflow_transformations(
        self,
        workflow_id: str,
        workflow_version_id: str,
        list_options: Optional[WorkflowTransformationListOptions] = None,
        max_results: int = None,
    ) -> Iterator[WorkflowTransformation]:
        return ResultIterator(
            WorkflowTransformationsListResultLoader(
                service_url=urljoin(
                    self.endpoint.url,
                    f"{self.namespace}/workflows/{workflow_id}/versions/{workflow_version_id}/transformations",
                ),
                http_session=self.create_http_session(),
                list_options=list_options,
                max_results=max_results,
    ...
    def get_workflow_transformation(
        self, workflow_id: str, workflow_version_id: str, transformation_id: str, trace: Optional[Span] = None
    ) -> WorkflowTransformation:
        ...
        with self.create_http_session() as session:
            response = session.get(
                urljoin(
                    self.endpoint.url,
                    f"{self.namespace}/workflows/{workflow_id}/versions/{workflow_version_id}/transformations/{transformation_id}",
                ),
                trace_context=trace,
            )
    ...
    def create_workflow_transformation(
        self,
        workflow_id: str,
        workflow_version_id: str,
