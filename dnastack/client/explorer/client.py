...
EXPLORER_SERVICE_TYPE_V1_0 = ServiceType(group="com.dnastack.explorer", artifact="collection-service", version="1.0.0")
...
class ExplorerClient(BaseServiceClient):
    ...
    def list_federated_questions(self, trace: Optional[Span] = None) -> "ResultIterator[FederatedQuestion]":
        ...
        return ResultIterator(
            loader=FederatedQuestionListResultLoader(
                service_url=urljoin(self.url, "questions"), http_session=self._session, trace=trace
            )
        )
    ...
    def describe_federated_question(self, question_id: str, trace: Optional[Span] = None) -> "FederatedQuestion":
        ...
        with self._session as session:
            try:
                ...
                if status_code == 401:
                    raise UnauthenticatedApiAccessError(f"Authentication required to access question '{question_id}'")
                elif status_code == 403:
                    raise UnauthorizedApiAccessError(f"Not authorized to access question '{question_id}'")
                elif status_code == 404:
                    raise ClientError(f"Question '{question_id}' not found")
                else:
                    raise ClientError(f"Failed to retrieve question '{question_id}': {e.response.text}")
    ...
    def ask_federated_question(
        ...
        request_payload = FederatedQuestionQueryRequest(inputs=inputs, collections=collections)
    ...
    def load(self) -> "List[FederatedQuestion]":
        ...
        with self.__http_session as session:
            try:
                ...
                status_code = e.response.status_code
                if status_code == 401:
                    raise UnauthenticatedApiAccessError("Authentication required to list federated questions")
                elif status_code == 403:
                    raise UnauthorizedApiAccessError("Not authorized to list federated questions")
                else:
                    raise ClientError(f"Failed to load federated questions: {e.response.text}")
        ...
        with self.__http_session as session:
            try:
                response = session.post(
                    self.__service_url, json=self.__request_payload.dict(), trace_context=self.__trace
                )
                ...
                status_code = e.response.status_code
                if status_code == 401:
                    raise UnauthenticatedApiAccessError("Authentication required to ask federated questions")
                elif status_code == 403:
                    raise UnauthorizedApiAccessError("Not authorized to ask federated questions")
                elif status_code == 400:
                    raise ClientError(f"Invalid question parameters: {e.response.text}")
                else:
                    raise ClientError(f"Failed to execute federated question: {e.response.text}")
