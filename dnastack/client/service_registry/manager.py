...
from dnastack.client.models import EndpointSource, ServiceEndpoint
from dnastack.client.service_registry.client import (
...
class ServiceRegistryManager:
    ...
    def add_registry_and_import_endpoints(self, registry_endpoint_id: str, registry_url: str) -> Context:
        ...
        identical_registry_endpoint_ids = [
            endpoint.id
            for endpoint in self.__context.endpoints
            if (endpoint.url == registry_url and endpoint.type == STANDARD_SERVICE_REGISTRY_TYPE_V1_0)
        ]
        if identical_registry_endpoint_ids:
            raise EndpointAlreadyExists(
                f"This URL ({registry_url}) has already been registered locally with the "
                f"following ID(s): {', '.join(identical_registry_endpoint_ids)}"
            )

        # Now, create a new endpoint.
        registry_endpoint = ServiceEndpoint(
            id=registry_endpoint_id, url=registry_url, type=STANDARD_SERVICE_REGISTRY_TYPE_V1_0
        )
    ...
    def synchronize_endpoints(self, registry_endpoint_id: str) -> Context:
        filtered_endpoints = [
            endpoint
            for endpoint in self.__context.endpoints
            if (endpoint.id == registry_endpoint_id and endpoint.type == STANDARD_SERVICE_REGISTRY_TYPE_V1_0)
        ]
    ...
    def __synchronize_endpoints_with(self, registry: ServiceRegistry) -> Context:
        ...
        sync_operations: Dict[str, _SyncOperation] = {
            endpoint.id: _SyncOperation(action="keep", endpoint=endpoint) for endpoint in endpoints
        }
        ...
        for service_entry in factory.all_service_infos():
            ...
            endpoint = parse_ga4gh_service_info(
                service_info, service_info.id if self.__in_isolation else f"{registry.endpoint.id}:{service_info.id}"
            )
            endpoint.source = EndpointSource(source_id=registry.endpoint.id, external_id=service_info.id)
            sync_operations[endpoint.id] = _SyncOperation(
                action="update" if endpoint.id in sync_operations else "add", endpoint=endpoint
            )
        ...
        for client_class in DATA_SERVICE_CLIENT_CLASSES:
            ...
            if short_type in default_mapping:
                ...
                if default_endpoint_id in similar_endpoints:
                    self.__logger.info(
                        f'The defined default "{short_type}" endpoint is still available and will be left untouched.'
                    )
                else:
                    del default_mapping[short_type]
                    self.__logger.debug(
                        f'The defined default "{short_type}" endpoint is no longer available and has been unset.'
                    )
            ...
            if short_type not in default_mapping:
                if len(similar_endpoints) == 1:
                    ...
                    self.__logger.info(
                        f'The default "{short_type}" endpoint will not be set as there exists '
    ...
    def remove_endpoints_associated_to(self, registry_endpoint_id: str) -> Context:
        ...
        for endpoint in endpoints:
            if endpoint.id == registry_endpoint_id or (
                endpoint.source and endpoint.source.source_id == registry_endpoint_id
            ):
