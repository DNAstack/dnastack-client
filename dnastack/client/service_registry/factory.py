...
from typing import Any, Dict, Iterable, Iterator, List, Optional, Set, Type, TypeVar, Union
...
class UnregisteredServiceEndpointError(ServiceEndpointNotFound):
    ...
    def __init__(self, services: Iterable[Service]):
        alternative_endpoint_urls = ", ".join(
            sorted(
                [
        ...
        super(UnregisteredServiceEndpointError, self).__init__(
            f"Try alternative(s): {alternative_endpoint_urls}" if alternative_endpoint_urls else "No alternatives"
        )
...
class ClientFactory:
        ...
        for registry in self.__registries:
            ...
            try:
                for service in registry.list_services():
                    entries.append(RegisteredServiceInfo(source_url=registry.url, info=service))
            except Exception:
                ...
                if auth_info.get("type") is None or auth_info.get("type") == "oauth2":
                    if "resource" not in auth_info:
                        self.__logger.warning(
                            f"R/{entry.source_url}: S/{entry.info.id}: Skipped from merging as The "
                            f"resource URL is not specified."
                        )
                        continue
                else:
                    self.__logger.warning(
                        f"R/{entry.source_url}: S/{entry.info.id}: Skipped from merging as it is not OAuth2."
                    )
                    continue
    ...
    def _merge_auth_info_per_group(self, auth_info_list: List[Dict[str, Any]]):
        ...
        for auth_info in auth_info_list:
            ...
            url_to_scopes_map[resource_url].update(
                auth_info["scope"].split(" ") if auth_info.get("scope") else [ALL_SCOPES]
            )
    ...
    def _make_auth_info_group_key(self, auth_info: Dict[str, Any]) -> str:
        content = dumps({k: v for k, v in auth_info.items() if k not in ["resource", "scope"]}, sort_keys=True)
        h = hashlib.new("sha256")
        h.update(content.encode("utf-8"))
        return h.hexdigest()

    def find_services(
        self, url: Optional[str] = None, types: Optional[List[ServiceType]] = None, exact_match: bool = True
    ) -> Iterable[Service]:
        """Find GA4GH services"""
    ...
    def get_service_endpoint_by_url(
        self, client_class: Type[SERVICE_CLIENT_CLASS], service_endpoint_url: str
    ) -> ServiceEndpoint:
    ...
    def get(self, id: str) -> Union[CollectionServiceClient, DataConnectClient, DrsClient]:
        ...
        client = EndpointRepository(
            SimpleStream(self.all_service_infos())
