...
class UnsupportedServiceTypeError(RuntimeError):
    ...
    def __init__(self, endpoint: ServiceEndpoint):
        super().__init__(
            f"{endpoint.id}: {endpoint.type.group}:{endpoint.type.artifact}:{endpoint.type.version} is not supported"
        )


def create(
    endpoint: ServiceEndpoint, additional_service_client_classes: Iterable[Type[BaseServiceClient]] = None
...
class SingleEndpointIdentifyingCriteriaError(RuntimeError):
    ...
    def __str__(self):
        ...
        if len(self.endpoints) == 0:
            ...
            simplified_service_list = (
                SimpleStream(self.endpoints).map(lambda endpoint: f"{endpoint.id} ({endpoint.type})").to_list()
            )
...
class EndpointRepository:
    ...
    def all(
        self, *, endpoint_type: Optional[ServiceType] = None, client_class: Optional[Type[BaseServiceClient]] = None
    ) -> List[ServiceEndpoint]:
        if endpoint_type is None and client_class is None:
            return self.__endpoints
        else:
            return (
                SimpleStream(self.__endpoints)
    ...
    def get_one_of(
        ...
        if len(target_endpoints) == 1:
            ...
            raise SingleEndpointIdentifyingCriteriaError(
                endpoints=target_endpoints, endpoint_type=endpoint_type, client_class=client_class
            )

    @staticmethod
    def __check_endpoint_compatibility(
    ...
    def __set_endpoints(self, endpoints: Iterable[ServiceEndpoint]):
        return [e for e in endpoints] if (self.__cacheable and not isinstance(endpoints, list)) else endpoints
