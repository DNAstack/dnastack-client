...
DATA_CONNECT_TYPE_V1_0 = ServiceType(group="org.ga4gh", artifact="data-connect", version="1.0.0")
...
class PageableResultLoader(ResultLoader):
    def __init__(self, initial_url: str, http_session: Optional[HttpSession] = None):
...
class TableListLoader(PageableResultLoader):
    def load(self) -> List[TableInfo]:
        ...
        with self._http_session as session:
            ...
            try:
                ...
                if status_code == 401:
                    ...
                    raise DataConnectError(
                        f"Unexpected error: {response_text}", status_code, response_text, urls=self._visited_urls
                    )
            ...
            try:
                ...
                raise DataConnectError(
                    f"Invalid Response Body: {response_body}", status_code, response_text, urls=self._visited_urls
                )
...
class DataMapper:
    ...
    @classmethod
    def init(cls, str_pattern: re.Pattern, map: Callable[[Any], Any]):
...
class IntervalDayToSecondMapper(DataMapper):
    def __init__(self):
        super().__init__(
            re.compile(r"P((?P<days>\d+)D)?(T((?P<hours>\d+)H)?((?P<minutes>\d+)M)?((?P<seconds>\d+)S)?)?"), self._map
        )

    def _map(self, s: Any) -> Any:
        raw_delta = self.str_pattern.match(s).groupdict()
        return timedelta(**{p: int(v) if v is not None else 0 for p, v in raw_delta.items()})
...
class UnexpectedRequestError(RuntimeError):
    """Unexpected Request Error"""

    def __init__(self, current_url: str, visited_urls: List[str], response: Response, trace: Span):
        super().__init__()
        self._current_url = current_url
        self._visited_urls = deepcopy(visited_urls)
    ...
    def __str__(self):
        return "\n".join(self._get_request_sequence())

    def _get_http_response_summary(self) -> str:
        status_code = self._response.status_code
        feedback = self._response.text
        return f"HTTP {status_code}: {feedback or ''} ({len(feedback)} B)"

    def _get_request_sequence(self) -> List[str]:
        sequence = []
        for i in range(len(self._visited_urls)):
            visited_url = self._visited_urls[i]
            sequence.append(f"Request #{i} to {visited_url}")
        sequence.append(f"Response: {self._get_http_response_summary()}")
        return sequence


class InvalidQueryError(UnexpectedRequestError):
    """Invalid Query Error"""

    def __init__(self, current_url: str, visited_urls: List[str], response: Response, query: str, trace: Span):
        super().__init__(current_url, visited_urls, response, trace)
        self._query = query

    def __str__(self):
        return f"{self._query} --> {self._get_http_response_summary()}"


class InvalidDataLoadingError(UnexpectedRequestError):
    ...
    def __str__(self):
        ...
        for i in range(len(self._visited_urls)):
            visited_url = self._visited_urls[i]
            sequence.append(f"Response #{i} from {visited_url}")
        sequence.append(
            f"Response #{len(self._visited_urls)}: HTTP {status_code}: {feedback or ''} ({len(feedback)} B)"
        )
        return "\n".join(sequence)


class QueryLoader(PageableResultLoader):
    _data_mapper_groups: Iterable[DataMapperGroup] = [
        # numbers
        DataMapperGroup("string", ["bigint"], [DataMapper.init(re.compile(r"^\d+$"), lambda s: int(s))]),
        DataMapperGroup(
            "string", ["decimal"], [DataMapper.init(re.compile(r"^(\d+\.\d+|\d+|\.\d+)$"), lambda s: Decimal(s))]
        ),
        # date
        DataMapperGroup(
            "string",
            ["date"],
            [
                DataMapper.init(re.compile(r"^\d{4}-\d{2}-\d{2}$"), lambda s: date.fromisoformat(s)),
            ],
        ),
        ...
        DataMapperGroup(
            ...
            [
                DataMapper.init(re.compile(r"^\d{2}:\d{2}:\d{2}(\.\d{,6})?$"), lambda s: time.fromisoformat(s)),
            ],
        ),
        # time with time zone
        DataMapperGroup(
            ...
            [
                DataMapper.init(
                    re.compile(r"^\d{2}:\d{2}:\d{2}(\.\d{,6})?Z$"), lambda s: time.fromisoformat(s[:-1] + "+00:00")
                ),
                DataMapper.init(
                    re.compile(r"^\d{2}:\d{2}:\d{2}(\.\d{,6})?\s*(-|\+)\d{2}$"), lambda s: time.fromisoformat(s + ":00")
                ),
                DataMapper.init(
                    re.compile(r"^\d{2}:\d{2}:\d{2}(\.\d{,6})?\s*(-|\+)\d{2}:\d{2}$"), lambda s: time.fromisoformat(s)
                ),
            ],
        ),
        ...
        DataMapperGroup("string", ["interval day to second"], [IntervalDayToSecondMapper()]),
    ]
        ...
        with self._http_session as session:
            try:
                if not self._current_url:
                    ...
                    if self.__query:
                        ...
                        response = session.post(
                            self._initial_url, json=dict(query=self.__query), trace_context=self.__trace
                        )
                    else:
                        # Fetch the table data
                        self.logger.debug(f"Initial Page: URL: {self._initial_url}")
                        response = session.get(self._initial_url, trace_context=self.__trace)
                    self._visited_urls.append(self._initial_url)
                else:
                    # Load a follow-up page.
                    self.logger.debug(f"Follow-up: URL: {self._current_url}")
                    response = session.get(self._current_url, trace_context=self.__trace)
                    ...
                    if not self._current_url:
                        if self.__query:
                            raise InvalidQueryError(
                                self._initial_url, query=self.__query, **common_error_properties
                            ) from e
                        else:
                            raise InvalidDataLoadingError(self._initial_url, **common_error_properties) from e
                    else:
                        if self.__query:
                            raise InvalidQueryError(
                                self._current_url, query=self.__query, **common_error_properties
                            ) from e
                        else:
                            raise InvalidDataLoadingError(self._current_url, **common_error_properties) from e
                else:
                    raise UnexpectedRequestError(self._current_url, **common_error_properties) from e
            except HttpError as e:
                status_code = e.response.status_code
                ...
                if status_code == 401:
                    ...
                    raise DataConnectError(error_feedback, status_code, response_body, urls=self._visited_urls) from e
            ...
            for mapper in mapper_group.mappers:
                ...
                try:
                    ...
                    raise DataConversionError(
                        f"{mapper_group}#{mapper_index}: Unexpected error during data "
...
class Table:
    ...
    def __init__(self, table_name: str, url: str, http_session: Optional[HttpSession] = None):
    ...
    def data(self) -> Iterator[Dict[str, Any]]:
        ...
        return ResultIterator(QueryLoader(http_session=self.__http_session, initial_url=urljoin(self.__url, "data")))
...
class DataConnectClient(BaseServiceClient):
    ...
    def query(self, query: str, no_auth: bool = False, trace: Optional[Span] = None) -> Iterator[Dict[str, Any]]:
        """Run an SQL query"""
    ...
    def iterate_tables(self, no_auth: bool = False) -> Iterator[TableInfo]:
        ...
        return ResultIterator(
            TableListLoader(
                http_session=self.create_http_session(no_auth=no_auth), initial_url=urljoin(self.url, r"tables")
            )
        )
    ...
    def table(self, table: Union[TableInfo, Table, str], no_auth: bool = False) -> Table:
        ...
        return Table(http_session=self.create_http_session(no_auth=no_auth), table_name=table_name, url=table_url)
