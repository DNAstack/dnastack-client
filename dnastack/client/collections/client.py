...
from dnastack.client.collections.model import (
    Collection,
    CollectionItem,
    CollectionItemListOptions,
    CollectionItemListResponse,
    CollectionStatus,
    CreateCollectionItemsRequest,
...
STANDARD_COLLECTION_SERVICE_TYPE_V1_0 = ServiceType(
    group="com.dnastack", artifact="collection-service", version="1.0.0"
)
STANDARD_DATASOURCE_SERVICE_TYPE_V1_0 = ServiceType(
    group="com.dnastack", artifact="collection-service", version="1.0.0"
)
...
EXPLORER_COLLECTION_SERVICE_TYPE_V1_0 = ServiceType(
    group="com.dnastack.explorer", artifact="collection-service", version="1.0.0"
)
    ...
    def load(self) -> List[any]:
        ...
        with self.__http_session as session:
            ...
            self.__next_page_url = (
                api_response.pagination.nextPageUrl
                if api_response.pagination and api_response.pagination.nextPageUrl
                else None
            )
            if not self.__next_page_url:
                self.__active = False
...
class CollectionServiceClient(BaseServiceClient):
    ...
    def create_collection(self, collection: Collection, trace: Optional[Span] = None) -> Collection:
        ...
        with self.create_http_session() as session:
            res = session.post(
                urljoin(self.url, "collections") + "?includeInternalItemsQuery=true",
    ...
    def list_collection_items(
        ...
        with self.create_http_session():
            return ResultIterator(
                CollectionItemListResultLoader(
                    service_url=urljoin(
                        self.endpoint.url, f"collection/{collection_id_or_slug_name_or_db_schema_name}/items"
                    ),
    ...
    def get_collection_status(
        self, collection_id_or_slug_name_or_db_schema_name: str, trace: Optional[Span] = None
    ) -> CollectionStatus:
        """Get the status of a collection"""
        trace = trace or Span(origin=self)
        with self.create_http_session() as session:
            res = session.get(
                urljoin(self.url, f"collection/{collection_id_or_slug_name_or_db_schema_name}/status"),
                trace_context=trace,
            )
    ...
    def create_collection_items(
        ...
        with self.create_http_session() as session:
            session.post(
                urljoin(self.url, f"collections/{collection_id_or_slug_name_or_db_schema_name}/items"),
                json=create_items_request.dict(),
                trace_context=trace,
            )
            return None
    ...
    def delete_collection(self, collection_id: str, trace: Optional[Span] = None) -> None:
        trace = trace or Span(origin=self)
    ...
    def data_connect_endpoint(
        self, collection: Union[str, Collection, None] = None, no_auth: bool = False
    ) -> ServiceEndpoint:
        """
        ...
        if (
            self._endpoint.dnastack_schema_version == 2.0
            and self._get_service_type() == STANDARD_COLLECTION_SERVICE_TYPE_V1_0
        ):
            ...
            if isinstance(collection, Collection):
                ...
                raise AssertionError(
                    f"For collection/{self._endpoint.dnastack_schema_version} ({self._endpoint.type}), the "
        ...
        if not no_auth and sub_endpoint.authentication:
            ...
            if not auth_type or auth_type == "oauth2":
                ...
                sub_endpoint.authentication["resource_url"] = (
                    self._endpoint.authentication.get("resource_url") or sub_endpoint.url
                )
