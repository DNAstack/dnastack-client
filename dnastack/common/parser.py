                ...
                if depth == last_depth:
                    ...
                    if p_name in node:
                        if isinstance(value, type(node[p_name])):
                            ...
                            self.__logger.error(error_message)
                            raise DotPropertiesAmbiguousStructureError(
                                key, ".".join([path[i] for i in range(depth + 1)])
                            )
                    # Now, set the value.
                    if is_array:
                    ...
                    if p_name in node and not isinstance(node.get(p_name), dict):
                        self.__logger.error(
                            f'Prevented structural change from "{type(node[p_name]).__name__}" (end of path) to "dict" (intermediate node) at "{key}"'
                        )
                        raise DotPropertiesAmbiguousStructureError(key, ".".join([path[i] for i in range(depth + 1)]))
