...
from dnastack.http.authenticators.abstract import Authenticator, AuthState, AuthStateStatus
...
class AuthManager:
    ...
    def revoke(self, endpoint_ids: List[str], confirmation_operation: Optional[Callable[[], bool]] = None) -> List[str]:
        ...
        for authenticator in authenticators:
            ...
            basic_event_info = dict(
                session_id=authenticator.session_id,
                index=index,
                total=total,
                state=state,
    ...
    def initiate_authentications(
        self, endpoint_ids: List[str] = None, force_refresh: bool = False, revoke_existing: bool = False
    ):
    ...
    def handle_block_response_required_event(self, event: Event):
        if event.details.get("kind") == "user_verification":
            self._logger.debug(
                f'Intercepting the "blocking-response-required" event for user verification ({event.details})...'
            )
            self.events.dispatch("user-verification-required", dict(url=event.details.get("url")))
        else:
            self._logger.error(
                f'Intercepted the "blocking-response-required" event but FAILED to handle {event.details}.'
            )

    def get_filtered_endpoints(self, endpoint_ids: List[str] = None) -> List[ServiceEndpoint]:
        return [endpoint for endpoint in self._context.endpoints if not endpoint_ids or endpoint.id in endpoint_ids]
