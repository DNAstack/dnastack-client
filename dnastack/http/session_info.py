...
class Session(SessionInfo):
    def __init__(self, *args, **kwargs):
        logging.warning(
            "dnastack.http.session_info.Session is deprecated. Please use "
...
@service.registered(
    params=[
        EnvironmentVariable(
            "DNASTACK_SESSION_DIR", default=os.path.join(LOCAL_STORAGE_DIRECTORY, "sessions"), allow_default=True
        )
    ]
)
...
class SessionManager:
    ...
    def __init__(
        ...
        if static_session_file and os.path.exists(static_session_file):
            ...
            with open(static_session_file) as f:
