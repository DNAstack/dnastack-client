...
from dnastack.http.authenticators.oauth2_adapter.abstract import AuthException, OAuth2Adapter
...
class DeviceCodeFlowAdapter(OAuth2Adapter):
    ...
    def exchange_tokens(self, trace_context: Span) -> Dict[str, Any]:
        ...
        with trace_context.new_span(metadata=trace_info) as sub_span:
            ...
            init_res = session.post(login_url, params=device_code_params, allow_redirects=False, headers=span_headers)
        ...
        while time() < expiry:
            with trace_context.new_span(metadata=trace_info) as sub_span:
                ...
                sub_logger.debug(
                    f"exchange_tokens: DC#2: {token_url}: HTTP {auth_token_res.status_code}:\n{auth_token_res.text}"
                )
