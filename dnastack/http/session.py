...
class RetryHistoryEntry(BaseModel):
    ...
    def __str__(self):
        return (
            f">>> Auth #{self.authenticator_index}\n"
...
class HttpSession(AbstractContextManager):
    ...
    def _session(self) -> Session:
        if not self.__session:
            ...
            self.__session.headers.update({"User-Agent": self.generate_http_user_agent()})
    ...
    def submit(
        ...
        **kwargs,
        ...
        logger.debug(
            f"{method.upper()} {url} {params or '(no params)'} (AUTH: {'Enabled' if self.__enable_auth else 'Disabled'})"
        )
            ...
            if self.__authenticators:
                if authenticator_index < len(self.__authenticators):
                    ...
                    raise AuthenticationError(
                        "Exhausted all authentication methods but still unable to get successful "
        ...
        if response.ok:
            ...
            if self.__enable_auth:
                ...
                if status_code == 401 and authenticator:
                    ...
                    self._raise_http_error(response, authenticator=authenticator, trace_context=trace_context)
            else:
    ...
    def get(self, url, trace_context: Optional[Span] = None, **kwargs) -> Response:
        return self.submit(method="get", url=url, trace_context=trace_context, **kwargs)

    def post(self, url, trace_context: Optional[Span] = None, **kwargs) -> Response:
        return self.submit(method="post", url=url, trace_context=trace_context, **kwargs)

    def delete(self, url, trace_context: Optional[Span] = None, **kwargs) -> Response:
        return self.submit(method="delete", url=url, trace_context=trace_context, **kwargs)

    def json_patch(self, url, trace_context: Optional[Span] = None, **kwargs) -> Response:
        headers = kwargs.setdefault("headers", {})
        headers["Content-Type"] = "application/json-patch+json"
        return self.submit(method="patch", url=url, trace_context=trace_context, **kwargs)
    ...
    def _raise_http_error(self, response: Response, authenticator: Authenticator, trace_context: Span):
        ...
        if isinstance(authenticator, OAuth2Authenticator):
            ...
            if last_known_session_info:
                ...
                if isinstance(parsed_response, dict) and parsed_response.get("error") == "invalid_token":
                    ...
                    if token:
                        trace_logger.error(
                            f"The token claims are {jwt.decode(token, options={'verify_signature': False})}."
                        )
                    else:
