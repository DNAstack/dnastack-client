...
import yaml
from imagination.decorator import EnvironmentVariable, service
...
@service.registered(
    params=[
        EnvironmentVariable(
            "DNASTACK_CONFIG_FILE", default=os.path.join(LOCAL_STORAGE_DIRECTORY, "config.yaml"), allow_default=True
        )
    ]
)
class ConfigurationManager:
    ...
    def load(self) -> Configuration:
        ...
        try:
            ...
            raise InvalidExistingConfigurationError(
                f"The existing configuration file at {self.__file_path} is invalid."
            ) from e
    ...
    def migrate(cls, configuration: Configuration) -> Configuration:
        ...
        if configuration.version == 3:
            ...
            default_context = Context(defaults=configuration.defaults, endpoints=configuration.endpoints)
