...
from dnastack.cli.commands.workbench.storage.utils import (
    validate_and_load_service_account_json,
    validate_azure_credentials,
)
from dnastack.cli.commands.workbench.utils import NAMESPACE_ARG, get_storage_client
...
from dnastack.client.workbench.storage.models import (
    AwsStorageAccountCredentials,
    AzureStorageAccountCredentials,
...
def update_aws_storage_account(
    ...
    storage_account = StorageAccount(
        id=storage_id, name=name, provider=Provider.aws, bucket=bucket, credentials=credentials
    )
...
def update_gcp_storage_account(
    context: Optional[str],
    endpoint_id: Optional[str],
    namespace: Optional[str],
    storage_id: str,
    ...
    credentials = GcpStorageAccountCredentials(
        service_account_json=loaded_service_account, region=region, project_id=project_id
    )
    storage_account = StorageAccount(
        id=storage_id, name=name, provider=Provider.gcp, bucket=bucket, credentials=credentials
    )
...
def update_azure_storage_account(
    context: Optional[str],
    endpoint_id: Optional[str],
    namespace: Optional[str],
    storage_id: str,
    ...
    try:
        credentials_type = validate_azure_credentials(
            sas=sas, access_key=access_key, tenant_id=tenant_id, client_id=client_id, client_secret=client_secret
        )
    ...
    storage_account = StorageAccount(
        id=storage_id, name=name, provider=Provider.azure, bucket=container, credentials=credentials
    )
