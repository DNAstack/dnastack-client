...
from dnastack.cli.commands.workbench.workflows.utils import JavaScriptFunctionExtractor, get_workflow_client
...
from dnastack.client.workbench.workflow.models import WorkflowTransformationCreate, WorkflowTransformationListOptions
...
def list_workflow_transformations(
    context: Optional[str], endpoint_id: Optional[str], namespace: Optional[str], workflow_id: str, version_id: str
):
    """
    ...
    transformations_list = client.list_workflow_transformations(
        workflow_id=workflow_id, workflow_version_id=version_id, list_options=list_options
    )
...
def describe_workflow_transformation(
    ...
    client = get_workflow_client(context_name=context, endpoint_id=endpoint_id, namespace=namespace)
    described_transformations = [
        client.get_workflow_transformation(
            workflow_id=workflow_id, workflow_version_id=version_id, transformation_id=transformation_id
        )
...
def delete_workflow_transformation(
    context: Optional[str],
    endpoint_id: Optional[str],
    namespace: Optional[str],
    workflow_id: str,
    ...
    transformation = client.get_workflow_transformation(
        workflow_id=workflow_id, workflow_version_id=version_id, transformation_id=transformation_id
    )
...
@formatted_command(
    ...
    specs=[
        ...
        ArgumentSpec(
            name="labels", arg_names=["--label"], help="A label to apply to the transformation", multiple=True
        ),
        CONTEXT_ARG,
        SINGLE_ENDPOINT_ID_ARG,
    ],
)
def add_workflow_transformation(
    context: Optional[str],
    endpoint_id: Optional[str],
    namespace: Optional[str],
    workflow_id: str,
    ...
    workflow_transformation = WorkflowTransformationCreate(
        id=transformation_id, next_transformation_id=next_transformation_id, script=script_content, labels=labels
    )

    response = client.create_workflow_transformation(
        workflow_id=workflow_id,
        workflow_version_id=version_id,
        workflow_transformation_create_request=workflow_transformation,
    )
