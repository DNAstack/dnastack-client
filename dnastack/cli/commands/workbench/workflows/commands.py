...
from dnastack.client.workbench.workflow.models import WorkflowCreate, WorkflowListOptions, WorkflowSource
...
def init_workflows_commands(group: Group):
    ...
    def describe_workflows(
        ...
        described_workflows = [
            workflows_client.get_workflow(workflow_id, include_deleted=include_deleted) for workflow_id in workflow_id
        ]
    ...
    @formatted_command(
        ...
        specs=[
            ...
            ArgumentSpec(
                ...
                help='Set a description for the workflow You can specify a file by prepending "@" to a path: @<path>',
                type=FileOrValue,
            ),
            ArgumentSpec(
                name="organization",
    ...
    def delete_workflow(
        ...
        if not force and not click.confirm(f'Do you want to delete "{workflow.name}"?'):
            return
