...
def init_questions_commands(group: Group):
    ...
    def list_questions(output: str, context: Optional[str], endpoint_id: Optional[str]):
        ...
        show_iterator(output_format=output, iterator=questions, transform=lambda q: q.dict())
    ...
    @formatted_command(
        ...
        specs=[
            ...
            ArgumentSpec(name="output_file", arg_names=["--output-file"], help="Output file path for results"),
            DATA_OUTPUT_ARG,
            CONTEXT_ARG,
            SINGLE_ENDPOINT_ID_ARG,
        ],
    ...
    def ask_question(
        ...
        results_iter = client.ask_federated_question(
            question_id=question_name, inputs=inputs, collections=collection_ids, trace=trace
        )
