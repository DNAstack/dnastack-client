...
def get_explorer_client(
    context: Optional[str] = None, endpoint_id: Optional[str] = None, trace: Optional[Span] = None
) -> ExplorerClient:
    """
...
def handle_question_results_output(results: List[Dict[str, Any]], output_file: Optional[str], output_format: str):
    ...
    if output_file:
        ...
        show_iterator(output_format=output_format, iterator=results)
...
def _write_yaml_results(results: List[Dict[str, Any]], output_file: str):
    ...
    with open(output_file, "w") as f:
        ...
        from yaml import dump as to_yaml_string
