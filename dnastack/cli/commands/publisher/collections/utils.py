...
def _switch_to_data_connect(
    ...
    try:
        proposed_data_connect_endpoint = collection_service_client.data_connect_endpoint(
            collection_id_or_slug_name, no_auth=no_auth
        )
            ...
            if proposed_data_connect_endpoint.type == endpoint.type:
                ...
                if proposed_data_connect_endpoint_url == reference_data_connect_endpoint_url:
                    return DataConnectClient.make(
                        endpoint.copy(update=default_no_auth_properties) if no_auth else endpoint
                    )
            else:
                pass
...
def _abort_with_collection_list(
    collection_service_client: CollectionServiceClient, collection_id_or_slug_name: Optional[str], no_auth: bool = False
):
    ...
    error_message = (
        f"The collection ID or slug name is not given via --collection or -c option or it is invalid. "
...
def _simplify_collection(collection: Collection):
    return {
        ...
        "tags": [tag.label for tag in collection.tags],
    }
...
def _filter_collection_fields(collection: Dict[str, Any], selected_fields: Optional[List[str]] = None):
    if not selected_fields:
        ...
        return {k: v for k, v in collection.items() if k in selected_fields}
