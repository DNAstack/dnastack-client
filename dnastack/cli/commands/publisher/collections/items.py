...
from dnastack.cli.commands.publisher.collections.utils import COLLECTION_ID_ARG, _get_collection_service_client
...
def list(
    ...
    assert limit >= 0, (
        "The limit (--limit) should be either ZERO (item query WITHOUT limit) or at least ONE (item query WITH limit)."
    )
    ...
    show_iterator(
        ...
        iterator=collection_service_client.list_collection_items(
            collection_id_or_slug_name_or_db_schema_name=collection, list_options=list_options, max_results=max_results
        ),
    )
...
def add_files_to_collection(collection: str, datasource: str, files: FileOrValue):
    ...
    try:
        ...
        client.create_collection_items(
            collection_id_or_slug_name_or_db_schema_name=collection, create_items_request=request
        )
...
def remove_files_from_collection(collection: str, datasource: str, files: FileOrValue):
    ...
    for file in parsed_files:
        ...
        try:
            client.delete_collection_items(
                collection_id_or_slug_name_or_db_schema_name=collection, delete_items_request=request
            )
        except ClientError as error:
