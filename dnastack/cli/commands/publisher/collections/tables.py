...
def list_tables(collection: Optional[str], no_auth: bool = False, output: Optional[str] = None):
...
@formatted_command(
    ...
    specs=[
        ArgumentSpec(
            name="table_name", arg_type=ArgumentType.POSITIONAL, help="The table name.", required=True, multiple=False
        ),
        COLLECTION_ID_ARG,
        RESOURCE_OUTPUT_ARG,
        CONTEXT_ARG,
...
def get_table_info(
    ...
    """List all accessible tables"""
    client = _switch_to_data_connect(
        _get_context(context), _get_collection_service_client(context, endpoint_id), collection, no_auth=no_auth
    )
