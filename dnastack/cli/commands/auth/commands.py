...
from dnastack.cli.commands.auth.event_handlers import (
    handle_auth_begin,
    handle_auth_end,
    handle_no_refresh_token,
    handle_refresh_skipped,
    handle_revoke_begin,
    handle_revoke_end,
)
from dnastack.cli.core.command import formatted_command
from dnastack.cli.core.command_spec import CONTEXT_ARG, RESOURCE_OUTPUT_ARG, SINGLE_ENDPOINT_ID_ARG, ArgumentSpec
...
def init_auth_commands(group: Group):
    ...
    def login(
        ...
        handler.initiate_authentications(
...
class AuthCommandHandler:
    ...
    def revoke(self, endpoint_ids: List[str], no_confirmation: bool):
        ...
        affected_endpoint_ids: List[str] = auth_manager.revoke(
            endpoint_ids,
            confirmation_operation=(None if no_confirmation else lambda: click.confirm("Do you want to proceed?")),
        )

        echo_header("Summary")

        if affected_endpoint_ids:
            echo_list("The client is no longer authenticated to the follow endpoints:", affected_endpoint_ids)
        else:
