...
from dnastack.cli.helpers.printer import echo_dict_in_table, echo_header, echo_list, echo_result
...
def handle_auth_begin(event: Event):
    ...
    if state.status != AuthStateStatus.READY:
        echo_result("Session", "yellow", "initializing", f"Session {session_id}", " ")
...
def handle_auth_end(event: Event):
    ...
    if state.status == AuthStateStatus.READY:
        ...
        if not is_refreshable:
            ...
            echo_header(
                f"Your session will expire in {duration_label.strip()}. Once it expires, you will "
                "be asked to log in again. ",
                bg="yellow",
            )

        echo_result("Session", _status_color_map[state.status], state.status, f"Session {session_id}", "●")

        feedback["refreshable"] = is_refreshable
        feedback["access_expires_in_seconds"] = access_grant_duration_in_seconds
    else:
        echo_result("Session", _status_color_map[state.status], state.status, f"Session {session_id}", "x")
...
def handle_revoke_begin(event: Event):
    session_id = event.details["session_id"]
    echo_result("Session", "yellow", "removing", f"Session {session_id}", " ")
...
def handle_revoke_end(event: Event):
    ...
    echo_result("Session", "red" if successfully_removed else "magenta", result, f"Session {session_id}", "x")
