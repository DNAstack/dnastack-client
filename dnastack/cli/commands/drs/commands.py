...
from dnastack.client.drs import DownloadFailureEvent, DownloadOkEvent, DownloadProgressEvent
...
def init_drs_commands(group: Group):
    ...
    def download(
        ...
        if not full_output:
            drs._download_files(id_or_urls=download_urls, output_dir=output_dir, no_auth=no_auth)
        else:
