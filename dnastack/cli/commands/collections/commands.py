...
def init_collections_commands(group: Group):
    ...
    def list_collections(
        ...
        show_iterator(
            ...
            [
                _filter_collection_fields(
                    _simplify_collection(collection) if simplified else collection, selected_fields
                )
    ...
    def list_items(
        ...
        assert limit >= 0, (
            "The limit (--limit) should be either ZERO (item query WITHOUT limit) "
            "or at least ONE (item query WITH limit)."
        )

        click.secho(
            f"Retrieving upto {limit} item{'s' if limit == 1 else ''}..." if limit else "Retrieving all items...",
            dim=True,
            err=True,
        )
        ...
        data_connect_client = _switch_to_data_connect(
            _get_context(context), collection_service_client, actual_collection.slugName, no_auth
        )
        ...
        def __simplify_item(row: Dict[str, Any]) -> Dict[str, Any]:
            ...
            item = dict(
                id=row["id"],
                name=row.get("qualified_table_name")
            ...
            item.update({k: v for k, v in row.items() if k in property_names})
        ...
        if re.search(r" limit\s*\d+$", items_query, re.IGNORECASE):
            ...
            limit_override = True
            items_query = (
                f"{items_query} LIMIT {limit + 1}"  # We use +1 as an indicator whether there are more results.
            )
        ...
        displayed_item_count = show_iterator(output or RESOURCE_OUTPUT_ARG.default, items, __simplify_item, limit)

        click.secho(
            f"Displayed {displayed_item_count} item{'s' if displayed_item_count != 1 else ''} from this collection",
            fg="green",
            err=True,
        )
        ...
        if limit_override and row_count > limit:
            click.secho(
                f"There exists more than {limit} item{'s' if limit != 1 else ''} in this collection. You may use "
