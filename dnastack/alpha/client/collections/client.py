...
from dnastack.client.collections.model import COLLECTION_READ_ONLY_PROPERTIES, Collection
        ...
        **attrs,
        ...
        if id is not None:
            if isinstance(collection, Collection):
                raise AmbiguousArgumentsError(
                    "Only require either a collection ID OR object, but not both, at the same time."
                )
            if not attrs:
        ...
        if not update_patches:
            raise NoUpdateError(
                f"No updates are required at the moment. It is possible that you are attempting to "
        ...
        with self.create_http_session() as session:
            ...
            patch_response = session.json_patch(
                resource_url, trace_context=trace, headers={"If-Match": etag}, json=update_patches
            )
