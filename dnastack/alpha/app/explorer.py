...
class Collection(PerCollectionApiMixin, BlobApiMixin):
    ...
    def __init__(
        self, factory: EndpointRepository, cs: CollectionServiceClient, collection: CollectionModel, no_auth: bool
    ):
    ...
    def get_filter_info(self, table_name: str, filters={}) -> FilterInfo:
        ...
        url = urljoin(
            endpoint.url,
            f"collections/{self._collection.slugName}/tables/{table_name}/filter/query?includeSharedQueryUrl=true",
        )
    ...
    def data_connect(self):
        ...
        proposed_data_connect_endpoint = self._cs.data_connect_endpoint(
            self._collection.slugName, no_auth=self._no_auth
        )
