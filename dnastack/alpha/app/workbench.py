...
class Workbench:
    ...
    def get_workflow(
        self,
        workflow_name: str,
        workflow_type: Optional[WorkflowSource] = __CUSTOM_WORKFLOW_SOURCE_NAME,
        max_results: Optional[int] = __WORKFLOW_MAX_RESULTS,
    ) -> Workflow:
        for workflow in self._get_workflow_client().list_workflows(
            list_options=WorkflowListOptions(source=workflow_type), max_results=max_results
        ):
    ...
    def list_runs(
        self, list_options: Optional[ExtendedRunListOptions] = None, max_results: int = None
    ) -> Iterator[ExtendedRunStatus]:
    ...
    def cancel_batch(self, batch_id: str) -> RunStatus:
        ...
        if len(bad_run_ids) > 0:
            raise WorkbenchRunException(
                "Could not cancel all runs in batch " + batch_id + ".  Run IDs: " + "\n".join(bad_run_ids),
                run_ids=bad_run_ids,
            )
    ...
    def _get_unanimous_state(self, batch_id: str) -> RunStatus:
        ...
        for run in runs_in_batch:
            ...
            if this_runs_state.has_failed():
                batch_error_message = batch_error_message + f"\nRun {run.run_id} failed with status {run.state}"
            elif this_runs_state.was_canceled():
                batch_error_message = batch_error_message + f"\nRun {run.run_id} was canceled"
        ...
        if batch_error_message != "":
            raise WorkbenchBatchException(
                "Batch failed because one or more runs failed or was canceled: " + batch_error_message, [batch_id]
            )

        return current_unanimous_state

    # polls a batch job until all jobs reach a desired state
    async def poll_batch_status_until(
        self,
        batch_id: str,
        desired_state: RunStatus,
        poll_interval: Optional[int] = 2,
        on_state_change: Optional[Callable[[RunStatus, str], None]] = None,
    ):
        current_unanimous_state = None
    ...
    async def poll_run_status_until(
        self,
        run_id: str,
        desired_state: RunStatus,
        poll_interval: Optional[int] = 2,
        on_state_change: Optional[Callable[[RunStatus], None]] = None,
    ):
        last_state = None
        ...
        while current_state != desired_state:
            ...
            if current_state.has_failed():
                ...
                raise WorkbenchCancellationException(
                    "Run " + run.run_id + " was canceled with status " + run.state, [run.run_id]
                )
    ...
    async def gather_runs(
        ...
        for run_id in run_ids:
            task = asyncio.create_task(
                self.poll_run_status_until(run_id, desired_state, poll_interval, on_state_change), name=run_id
            )
            tasks.append(task)
        try:
            ...
            if result.run_ids_with_failure is not None and len(result.run_ids_with_failure):
                raise WorkbenchCancellationException(
                    f"Unable to cancel runs after failure/cancellation of run {result.run_ids_with_failure}"
                ) from ex
            else:
                raise ex
    ...
    def _get_workflow_url(self, workflow_id: str, version_id: str) -> str:
        return f"{workflow_id}/{version_id}"

    def scatter(
        self,
        workflow: Workflow,
        version: WorkflowVersion,
        engines: Union[List[str], dict] = None,
        run_constants=None,
        run_variables=None,
        tags={},
        batch_id=None,
        ...
        if isinstance(engines, list):
            ...
            raise Exception(
                "Engine information must be given as a list of engine ids, or a dict of engine-ids -> engine configuration"
            )

        if run_constants is not None and type(run_constants) is List:
            if len(run_constants) != len(engine_ids):
                raise Exception(
                    "Expected " + len(engine_ids) + " dicts of run constants, but only received " + len(run_constants)
                )
        ...
        for engine_index in range(0, len(engines)):
            ...
            batch_run_request = BatchRunRequest(
                ...
                engine_id=engine_id,
                default_workflow_params=run_constants[engine_index]
                if run_constants is not None and type(run_constants) is list
                else run_constants,
                default_workflow_engine_parameters=default_engine_params[engine_id]
                if engine_id in default_engine_params
                else None,
                default_tags=tags,
                run_requests=[
                    ExtendedRunRequest(**{"workflow_params": run_in_batch}) for run_in_batch in runs_in_batch
                ],
            )
    ...
    def stream_task_log_by_task(
        self, run: Union[ExtendedRun, str], task_name: str, log_type: LogType, max_bytes: Optional[int] = None
    ) -> Iterable[bytes]:
        if type(run) is ExtendedRun:
            run = run.run_id
        for task_log in run.task_logs:
            if task_log.name == task_name:
                return self.stream_task_log(
                    run_id=run, task_id=task_log.task_id, log_type=log_type, max_bytes=max_bytes
                )

    def stream_task_log(
        self, run: Union[ExtendedRun, str], task_id: str, log_type: LogType, max_bytes: Optional[int] = None
    ) -> Iterable[bytes]:
        return self._get_ewes_client().stream_task_logs(
            run_id=run, task_id=task_id, log_type=log_type, max_bytes=max_bytes
        )

    def stream_run_log(
        self, run: Union[ExtendedRun, str], log_type: LogType, max_bytes: Optional[int] = None
    ) -> Iterable[bytes]:
        if type(run) is ExtendedRun:
            run = run.run_id
